...
class CreateNewPassword : AppCompatActivity() {
    ...
    private fun forgotPassword(email: String, password: String) {
        ...
        api.forgotPassword(email, password).enqueue(object : retrofit2.Callback<ForgotPasswordResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    if (forgotPasswordResponse != null && forgotPasswordResponse.success) {
                        ...
                        val intent = Intent(this@CreateNewPassword, LoginPage::class.java)
                        startActivity(intent)
                        finish()
                    } else {
                        Log.e("CreateNewPassword","email: ${email}, password")
                        Toast.makeText(this@CreateNewPassword, forgotPasswordResponse?.message ?: "Failed to change password", Toast.LENGTH_SHORT).show()
                    }
                } else {
